...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uARK
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/661.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
{
    int64 nSubsidy = 101 * COIN;

    // Subsidy is cut in half every 110011 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 110011);
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1391688685, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xf266db57 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xf2, 0x66, 0xdb, 0x57 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArkhashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArkhashMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("ArkhashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arkhash-miner");
    ...
    try { loop {
        ...
        printf("Running ArkhashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArkhashMiner terminated\n");
        throw;
    }
}
