...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ARK")
    updateDisplayUnit();
}
...
{
    QString ret = QString("arkhash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ARK unit input in lnReqAmount, we generate the URI with ARK as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ARK, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
